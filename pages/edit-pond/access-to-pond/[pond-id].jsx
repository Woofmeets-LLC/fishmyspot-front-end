...
const AccessToPondEdit = () => {
  ...
  const getPondData = () => {
    setLoading(true);
    getSdk()
      .ownListings.show({ id: query['pond-id'], include: ['images'] })
      .then((res) => {
        setLoading(false);
  ...
  const handleUpdatePondData = async (values, helpers) => {
    const existingImages = pondData?.relationships?.images?.data
      ?.filter((image) =>
        values?.['ATP-images-base64']?.includes(
          image?.attributes?.variants?.default?.url
        )
      )
      ?.map((image) => image.id);

    const imageFilesForUpload = values?.['ATP-images-file']?.filter(
    ...
    const requestData = {
      ...
      publicData: {
        ...
        'additional-information-description':
          values?.['additional-information-description'],
      },
    };
    ...
    <HomeLayout>
      <EditPondContainer>
        <Formik
          ...
          initialValues={{
            'ATP-description':
              pondData?.attributes?.publicData?.[
                'access-to-pond-description'
              ] || '',
            'ATP-images-file':
              pondData?.relationships?.images?.data?.map(
